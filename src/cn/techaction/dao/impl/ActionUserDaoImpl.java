...
//System.out.println(rs.get(0).intValue());
...
//System.out.println(queryRunner.query(sql,new BeanHandler<User>(User.class),account,password));
	...
	public int updateUserInfo(User user) {
		String sql = "update action_users set update_time = ?,"
				+ "password = ?,email = ?,phone = ?, question = ?,asw = ?,name = ?,age = ?,sex = ?,"
				+ "create_time = ?,account = ?,role = ?,del = ? where id = ?";
	 ...
	 */
	@Override
	public int findPointById(Integer Id) {
		String sql = "select points from action_users where id = ?";
		try {
			List<Integer> rs = queryRunner.query(sql, new ColumnListHandler<Integer>("points"), Id);
			return rs.size()>0?rs.get(0).intValue():0;
		} catch (SQLException e) {
			e.printStackTrace();
			return 0;
	 ...
	 */
	@Override
	public int addPointById(Integer Id,Integer points) {
		points = points + this.findPointById(Id);
		String sql = "update action_users set points = ? where id = ?";	
		try {
			return queryRunner.update(sql, points ,Id);
		} catch (SQLException e) {
			e.printStackTrace();
			return 0;
		}
