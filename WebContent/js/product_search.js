		...
		$.ajax({
			...
			success:function(rs){
				...
				$("#productTypeContainer li a").click(function(){
					...
					//刷新导航栏
					$("#parentTag").html( $(this).attr("product-name") );
					//$("#parentTag").attr("href","product_search.html?parentId="+ptid);
					$("#childTag_1").html("&nbsp;");
					$("#childTag_2").html("&nbsp;");
...
//alert(partsData);
		...
		var func = Handlebars.compile(tpl);
		var model = $("#parts_type_container").html(
				"<a href='#' class='current_a partsType01' >不限</a>"
				+
				func(partsData));
		//设置选中时配件类型分类
		...
		$("#parts_type_container").find("a").click(function(){
			...
			//在这里进行上部导航栏刷新
			$("#childTag_1").html( $(this).attr("product-parts-name01") );
			//$("#childTag_1").attr("href","product_search.html?parentId="+productTid+"&partsId="+partsTid);
			$("#childTag_2").html( $(this).attr("product-parts-name02") );
